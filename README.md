...
- 一个平台打通站点、设备、故障、照片、统计、后台导入和工作记录闭环
- 支持台账导入、故障汇总导入、导入审查和批次结果追踪
- 支持多项目并行、项目访问控制和项目级故障类型版本管理
- 支持照片自动匹配站点、人工纠偏和缩略图混合存储
- 支持故障闭环后自动写入 `工作记录.xlsx`，并按历史工作记录风格生成故障描述
- 支持重复故障检测：前端提醒 + 后端 7 天未闭环硬拦截
- 支持站点详情页槽位/通道/摄像机编号一致化展示，以及录像机分组诊断
...
- 支持重复故障检测：同站点 + 同故障类型 7 天内未闭环故障可前端提醒、后端拦截
- 处理结果可沉淀在故障记录中，便于后续检索和统计
- 故障闭环后可自动同步到 `E:\办公\工作记录\工作记录.xlsx`
...
- 故障闭环后自动写入 `工作记录.xlsx`
- 自动映射时间、站点、地点、甲供、负责人等字段
- 故障描述优先保留现场处理备注，备注过泛时自动补摄像机点位
- 多摄像机故障会自动控制描述长度，尽量贴近既有工作记录风格
- 可与后续差旅报销、月度统计、工作复盘形成链路
...
- `/stations`
  ...
  - 站点详情支持项目 / 录像机 / 槽位分组诊断
  - 槽位号、通道号、摄像机编号按一致化口径展示
- `/fault/new`
  - 新建故障报修，支持项目、站点、摄像机等维度录入
  - 提交前可做重复故障检测提醒
- `/faults`
  - 故障记录列表、状态流转、处理与关闭
  - 聚合故障支持多摄像机摘要、恢复状态、详情时间线
- `/statistics`
...
├─ worklog_sync.py                 # 故障闭环 → 工作记录.xlsx 自动同步
├─ worklog_sync.md                 # 工作记录同步规则说明
...
- `POST /api/faults`
- `GET /api/faults`
- `GET /api/faults/duplicate-check`
